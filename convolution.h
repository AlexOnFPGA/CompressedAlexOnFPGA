...
void conv_bram_out (TYPE_T *img_in, TYPE_T fm_out[M][R_OUT][C_OUT], TYPE_T *weight, TYPE_T *bias){
...
#pragma HLS INTERFACE s_axilite port=return bundle=control
    	...
    	TYPE_T fm_in[Tn][R_IN+2*P][C_IN+2*P];
...
#pragma HLS ARRAY_RESHAPE variable=buf_weight complete dim=2
		...
		L1:for (int to=0; to < M; to+=Tm){
			L0:for (int ti=0; ti < N; ti+=Tn){
			 memcpy(&(fm_in[0][0][0]), img_in+ti*R_IN*C_IN, sizeof(TYPE_T)*Tn*(R_IN+2*P)*(C_IN+2*P)); //if fm_in need to be tiled
...
template<int R_IN, int C_IN, int N, int K, int M, int P, int S, int R_OUT, int C_OUT>
void conv_axi_out (TYPE_T *img_in, TYPE_T *conv_out, TYPE_T *weight, TYPE_T *bias){
#pragma HLS INTERFACE m_axi port=img_in offset=slave bundle=gmem1
#pragma HLS INTERFACE m_axi port=weight offset=slave bundle=gmem2
#pragma HLS INTERFACE m_axi port=bias offset=slave bundle=gmem2
...
#pragma HLS INTERFACE s_axilite port=return bundle=control
    	...
    	TYPE_T fm_in[N][R_IN+2*P][C_IN+2*P]; //buffer for img_in
	#pragma HLS ARRAY_RESHAPE variable=fm_in cyclic dim=1 factor=16
    //#pragma HLS ARRAY_RESHAPE variable=fm_in complete dim=1 // if fm_in tiled
    	memcpy(&(fm_in[0][0][0]), img_in, sizeof(TYPE_T)*N*(R_IN+2*P)*(C_IN+2*P));
    	TYPE_T buf_bias[M];
    #pragma HLS ARRAY_RESHAPE variable=buf_bias complete dim=1
    	 memcpy (buf_bias, bias, sizeof(TYPE_T)*M);
    	TYPE_T fm_out[M][R_OUT][C_OUT];
    #pragma HLS ARRAY_RESHAPE variable=fm_out cyclic dim=1 factor=32
...
#pragma HLS PIPELINE
             L10:for (int i = 0;i<M;i++){
                   fm_out[i][j][k]= buf_bias[i] + fm_out[i][j][k];
				   if (fm_out[i][j][k] < 0) fm_out[i][j][k] = 0;
                 }
           }
